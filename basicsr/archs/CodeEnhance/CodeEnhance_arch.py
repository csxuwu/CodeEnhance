...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self,
        ...
        self.codebook_shift = codebook_shift
        self.shift_codebook = nn.Parameter(torch.zeros(size=(self.codebook_emb_num, self.codebook_emb_dim)))   
        
        self.weight_shift = weight_shift # 0.001
        self.weight_shift_reg = weight_shift_reg
    ...
    def forward(self, z, gt_indices=None,  seg_feat=None, seg_feat_gt=None, current_iter=None, weight_alpha=None):
        """
        Args:
        ...
        z = z.permute(0, 2, 3, 1).contiguous()
        z_flattened = z.view(-1, self.codebook_emb_dim) # 1024 * 512
        ...
        # -------------------------------------------------------
        shift_reg = 0.0
        if self.codebook_shift:
            codebook = codebook + self.weight_shift * self.shift_codebook
            shift_reg = self.weight_shift_reg * torch.norm(self.shift_codebook, p=2)    # 正则化项
        ...
        if gt_indices is not None:
            ...
            z_q_gt = torch.matmul(gt_min_onehot, codebook)      # 获得标签图像的 量化特征
        ...
        if self.LQ_stage and gt_indices is not None:
            # loss of Stage II
            codebook_loss = self.dist(z_q_gt, z)
            texture_loss = self.gram_loss(z, z_q_gt)

            e_latent_loss = torch.mean((z_q_gt - z_q) ** 2)     

            codebook_loss = codebook_loss + texture_loss + e_latent_loss
        else:
            # loss of Stage I
        ...
        codebook_loss = codebook_loss + shift_reg
...
class CodeEnhance_arch(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
        ...
        self.codebook_scale = codebook_params[:, 0]             # 64，codebook 输入特征的尺寸
        codebook_emb_num = codebook_params[:, 1].astype(int)    # 1024，包含 1024 个量化特征
        codebook_emb_dim = codebook_params[:, 2].astype(int)    # 512，每个量化特征的维度是512
    ...
    def encode_and_decode(self, input, gt_img=None, reference_img=None, gt_indices=None, weight_alpha=None, net_hq=None):
        ...
        high_level_feats_gt = self._get_semantic_info(gt_img)
        semantic_gt_feat = high_level_feats_gt['seg_feats']
        ...
        # 当 reference image 存在，且 net_hq 也存在
        ...
        # 否则，用encoder 的feature替代 refer image feature
        else:
            ...
            if cur_res in self.codebook_scale:  # needs to perform quantize
                ...
                if gt_indices is not None:
                    z_quant, codebook_loss, indices = self.quantize_group[quant_idx](feat_to_quant, gt_indices[quant_idx],
                                                                                     seg_feat=semantic_feat,
                                                                                     seg_feat_gt=semantic_gt_feat,
                                                                                     weight_alpha=self.weight_alpha)
                else:
