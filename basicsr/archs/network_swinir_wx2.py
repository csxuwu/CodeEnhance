...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.aggre = nn.Sequential(nn.Conv2d(512, 256, 3, 1, 1), nn.ReLU())     # 512, 256 根据实际情况调整，这里为了方便就没有单独设置参数传入了
    ...
    def forward(self, x,  y, z, mask=None):
        ...
        k = self.k(z).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, z, x_size):
        ...
        if self.input_resolution == x_size:
            attn_windows = self.attn(x_windows, y_windows, z_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        else:
            attn_windows = self.attn(x_windows, y_windows, z_windows, mask=self.calculate_mask(x_size).to(x.device))
    ...
    def forward(self, x, y, z, x_size):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, y, z, x_size)
            else:
                x = blk(x, y, z, x_size)
...
class RSTB(nn.Module):
    ...
    def forward(self, x, y, z, x_size):
        ...
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, y, z, x_size), x_size))) + x
