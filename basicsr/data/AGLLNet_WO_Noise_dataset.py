...
class AGLLNet_WO_Noise_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(AGLLNet_WO_Noise_Dataset, self).__init__()
        self.opt = opt
        ...
        if self.opt['phase'] == 'train':
            ...
            self.imgs_ll = glob.glob(os.path.join(opt['dataroot'], 'test', '*.*'))
            self.imgs_gt = glob.glob(os.path.join(opt['dataroot'], 'test', '*.*'))
            self.img_size = opt.get('gt_size', 512)

            # print(os.path.join(opt['dataroot'], 'test', 'input', '*.*'))
            # print(len(self.imgs_ll))
            # import sys
            # sys.exit()
        self.imgs_ll.sort()
        self.imgs_gt.sort()
